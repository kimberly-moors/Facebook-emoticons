...
if __name__ == "__main__":
	...
	def KNN_model(train_f, test_f, y1, y2):
		...
		pan= model.best_estimator_
		ss = cross_val_score(model, train_f, y1, cv = cv)
		...
		F1 = f1_score(y2, pp)
		return('KNN results:', pan, 'crossvalidation =', ss, 'accuracy =', acc, 'confusion matrix =', conf, 'precision =', prec, 'recall =', rec, "roc =", roc, 'MAE =', MAE, 'F1 =', F1 )
	...
	def SVMs_model(train_f, test_f, y1, y2):
		...
		param_grid = {'C':[1,10]}
		cv = StratifiedKFold(n_splits=10, random_state=0)
		model = GridSearchCV(svm.SVC(gamma='scale'),param_grid, iid= True, cv=cv, refit = True, verbose=2, scoring='f1')
		model.fit(train_f,y1)
		pan = model.best_estimator_
		ss = cross_val_score(model, train_f, y1, cv = cv)
		pp = model.predict(test_f)
		...
		MAE = mean_absolute_error(y2, pp)
		F1 = f1_score(y2, pp)
		return('SVMs results:', pan, 'crossvalidation =', ss, 'accuracy =', acc, 'confusion matrix =', conf, 'precision =', prec, 'recall =', rec, "roc =", roc, 'MAE =', MAE, 'F1 =', F1 )
   ...
   #Logistic Regression
	def LR_model(train_f, test_f, y1, y2):
		...
		model = LogisticRegression(C = 1, solver = 'lbfgs', max_iter = 900)
		...
		MAE = mean_absolute_error(y2, pp)
		F1 = f1_score(y2, pp)
		return('LR results:', pan, 'crossvalidation =', ss, 'accuracy =', acc, 'confusion matrix =', conf, 'precision =', prec, 'recall =', rec, "roc =", roc, 'MAE =', MAE, 'F1 =', F1 )
	...
	print (KNN_model(emoji_train,emoji_test, y1, y2))
	print (SVMs_model(emoji_train, emoji_test, y1, y2))
	print (LR_model(emoji_train, emoji_test, y1, y2))

	#Bag of Words
	print ('BW FEATURIZER')
	vec = CountVectorizer(analyzer = 'word', max_features = 6000, min_df = 2)
	train_f = vec.fit_transform(train['STATUS'])
	train_f = (pd.DataFrame(train_f.toarray(), columns = vec.get_feature_names()))
	test_f = vec.transform(test['STATUS'])
	...
	dicty_train = dictff(train)
	train_l = dicty_train.reset_index(drop = True)
	dicty_test= dictff(test)
